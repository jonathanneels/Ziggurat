  ...
  let initKnobs=(el)=>{
    ...
    ik.pointerdown=(ev)=>{
      ...
      ik.dragfrom={x:ev.clientX,y:ev.clientY,a:Math.atan2(ev.clientX-cx,cy-ev.clientY),v:+el.value};
	        document.addEventListener("mousemove",ik.pointermove,false);
      document.addEventListener("mouseup",ik.pointerup,false);
	        ev.preventDefault();
      ev.stopPropagation(); 
		  }else{ 
      ik.dragfrom={x:ev.clientX,y:ev.clientY,a:Math.atan2(ev.clientX-cx,cy-ev.clientY),v:+el.value};
  
      document.addEventListener("touchmove",ik.pointermove,false);
      document.addEventListener("touchend",ik.pointerup,false);
      document.addEventListener("touchcancel",ik.pointerup,false);
      document.addEventListener("touchstart",ik.preventScroll,false);
 	  }
    };
    ik.pointermove=(ev)=>{
      let dv;
    ...
    ik.pointerup=()=>{
      document.removeEventListener("mousemove",ik.pointermove,false);
      document.removeEventListener("touchmove",ik.pointermove,false);
      document.removeEventListener("mouseup",ik.pointerup,false);
      document.removeEventListener("touchend",ik.pointerup,false);
      document.removeEventListener("touchcancel",ik.pointerup,false);
      document.removeEventListener("touchstart",ik.preventScroll,false);
      let event=document.createEvent("HTMLEvents");
